...
export default function VerifyEmailModal({
  ...
  const { confirmForgotPasswordOtp, sendForgotPasswordOtp, loading } = useAuth();
  ...
  const onSubmit = async (values: z.infer<typeof verifySchema>) => {
    try {
      // Gọi API xác nhận OTP quên mật khẩu
      const result = await confirmForgotPasswordOtp(values.email, values.code);
